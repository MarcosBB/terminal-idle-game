...
- **`main.py`** — wires `Game` and `Menu` together. Parses a `--reset` CLI flag (via `argparse`); when absent, calls `game.load()` right after constructing `Game` and before constructing `Menu`, so the loaded state is reflected in the first render. Registers a `pynput.keyboard.Listener` (`on_press`) that maps number keys to `buy_property(index)`, `x` to `change_multiplier`, `s` to `save`. Runs an infinite loop inside a `rich.live.Live` context: advances game state (`earn_money`), then calls `live.update(menu.render(), refresh=True)` — `Live` diffs and redraws only the changed region in place (no full-screen clear, unlike the old `os.system("clear")` approach, which caused visible flicker) — and sleeps to hold the frame rate at `SECONDS_PER_FRAME`, tracking `frame_rate_problem` if a frame overruns its budget. The frame-rate warning is printed via the shared `console.print(...)` (not bare `print`), since printing to the same `Console` a `Live` is using is what keeps output from corrupting the live region.
