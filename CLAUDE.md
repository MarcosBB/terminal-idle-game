...
A terminal-based idle/incremental game (like Cookie Clicker) built in Python. The player buys properties (farmer, cow, plantation, etc.) that generate passive income, rendered as a `rich` table refreshed in a loop via `rich.live.Live` (in-place, flicker-free redraw), with `pynput` capturing keypresses.
...
- **`src/configs.py`** — all tunable constants and the default game state: `DEFAULT_MONEY`, `DEFAULT_PROPERTIES` (list of property dicts with `name`/`value`/`income`/`quantity`/`money_per_second`), `MULTIPLIER_OPTIONS` (`[1, 10, 100, "Max"]`), `MAX_VALUE` (the sentinel string `"Max"` used as a multiplier option), `SAVE_FILE_PATH` (default `savegame.json`), `FRAMES_PER_SECOND`/`SECONDS_PER_FRAME` which drive the game-logic tick rate, and `DISPLAY_FRAMES_PER_SECOND`/`DISPLAY_SECONDS_PER_FRAME` which drive how often the screen is actually redrawn (lower than the tick rate, on purpose — see `main.py`).
...
- **`src/menu.py`** — `Menu` is purely presentational: builds the header string (money, money/sec, current multiplier) and a `rich.table.Table` of properties from `Game` state, formatting numbers with `numerize`. `render()` combines header/table/footer into a `rich.console.Group` for `Live` to display. It does not mutate `Game`.
- **`main.py`** — wires `Game` and `Menu` together. Parses a `--reset` CLI flag (via `argparse`); when absent, calls `game.load()` right after constructing `Game` and before constructing `Menu`, so the loaded state is reflected in the first render. Registers a `pynput.keyboard.Listener` (`on_press`) that maps number keys to `buy_property(index)`, `x` to `change_multiplier`, `s` to `save`. Runs an infinite loop inside a `rich.live.Live(..., auto_refresh=False)` context: `earn_money()` still ticks every frame at `SECONDS_PER_FRAME` for precision, but the screen is only redrawn (`menu.update_header()` + `live.update(menu.render(), refresh=True)`) when at least `DISPLAY_SECONDS_PER_FRAME` has elapsed since the last redraw — redrawing 24 times a second was visibly flickering, so the render rate is intentionally throttled below the tick rate. `auto_refresh` is off because `Live`'s own background auto-refresh thread and this manual per-frame `refresh=True` would otherwise both repaint independently and fight each other, which was the main source of the flicker. `Live` itself never does a full-screen clear (unlike the old `os.system("clear")` approach) — it repositions the cursor and rewrites only the live region. The frame-rate warning is printed via the shared `console.print(...)` (not bare `print`), since printing to the same `Console` a `Live` is using is what keeps output from corrupting the live region.
